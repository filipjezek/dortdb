...
SelectSet
  = 'SELECT'i _ dist:(@DistinctClause _)? _ sl:SelectList _ 'FROM'i _
  tbl:TableItem w:(_ @WhereClause)? gb:(_ @GroupByClause)? h:(_ @HavingClause)? wind:(_ @WindowClause)? {
    return new ast.SelectSet(sl, tbl, w, gb, h, !!dist, wind);
  }
  / 'SELECT'i dist:(_ @DistinctClause)? _ sl:SelectList {
...
BooleanLiteral
  = 'TRUE'i { return new ast.ASTLiteral(true); }
  / 'FALSE'i { return new ast.ASTLiteral(false); }
  / 'NULL'i { return new ast.ASTLiteral(null); }
  ;
